...
Karmastro = plateforme d'astrologie et numerologie francophone. 
Cible : utilisateurs curieux de spiritualite, astrologie moderne + datascience.

Usage:
  python3 scripts/serp_brief.py --max 10
  python3 scripts/serp_brief.py --index 8
  python3 scripts/serp_brief.py --force
"""
...
BRIEF_PROMPT_TEMPLATE = """Tu es un strategiste SEO et GEO (Generative Engine Optimization) specialise en niche : astrologie, numerologie, tarot, spiritualite moderne.
...
REQUETE : "{keyword}"
MARCHE CIBLE : France + Europe francophone (+ multilingue ES IT DE EN AR TR etc.), audience curieuse d'astrologie moderne, numerologie pratique, tarot, spiritualite data-driven
NOTRE POSITIONNEMENT : Karmastro, plateforme d'astrologie et numerologie qui combine tradition astrologique + rigueur editoriale + explications claires. Angle : demystifier sans desacraliser. Outils gratuits (theme natal, chemin de vie, compatibilite), contenu expert.
...
{{
  ...
  "citable_facts_to_verify": [
    "stat/fait astrologique/numerologique cite (source a verifier) 1",
    "fait precis 2",
    "fait precis 3"
  ],
  "entities_to_mention": ["nom entite 1 (astrologue connu, tradition, culture)", "entite 2", "entite 3"],
...
- Pour le domaine astro/numero : si des termes esoteriques sont utilises, les expliquer clairement pour les LLMs"""
...
def gemini_serp_brief(keyword: str, retries: int = 3) -> dict:
    ...
    for attempt in range(retries):
        try:
            r = requests.post(f"{GEMINI_URL}?key={GOOGLE_API_KEY}", json=payload,
...
def main():
    ...
    targets = []
    for a in articles:
        if args.index is not None and a.get("index") != args.index: continue
        if not args.force and a.get("serp_brief"): continue
